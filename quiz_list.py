...
for name in filenames:
    if name.endswith(".hpp") is True:
        newname = name.replace(".hpp",".h")
        newfilenames.append(newname)
    else: newfilenames.append(name)
print(newfilenames)
...
#text="hello how are you"
    say = []
    newsay = ""  # Separate the text into words
    words = text.split()
    for word in words:
    # Create the pig latin word and add it to the list
        newsay = word[1:] + word [0]
    #print(newsay)
        newsay = newsay + "ay"
    #print(newsay)
        say.append(newsay)
...
# For example:
# 640 is read/write for the owner, read for the group, and no permissions for
# the others; converted to a string, it would be: "rw-r-----"
